...
3. 폰을 **가로로, 화면 위쪽이 왼쪽을 향하게** 잡는다. 화면은 나를 향한다.
4. 로비에서 **A = 테니스**, **B = 포인터 테스트**.
5. 캘리브레이션 화면에서 정면을 겨눈 자세로 **A**. 그 자세가 중립이 된다.
...
**Phase 0~4 구현 완료.** 프로토콜·릴레이·센서 파이프라인·입력 레이어·테니스·재접속 복구·상보 필터.

남은 것은 **실기기 좌표계 확정**이다. 폰을 움직이며 오버레이의 canonical 부호가
`ARCHITECTURE.md` 5.6 표와 맞는지 확인해야 한다. 다르면 코드가 아니라 문서를 먼저 고친다.
