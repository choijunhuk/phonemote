...
USB 케이블, 카카오톡 "나에게 보내기", Google Drive 등 편한 방법이면 된다.
파일이 폰의 `다운로드` 폴더에 있으면 된다.

> Android가 `.pem` 확장자를 인식하지 못하면 파일명을 `rootCA.crt` 로 바꿔서 넣는다.

### 3.3 인증서 설치

**설정 → 보안 및 개인정보 보호 → 추가 보안 설정 → 암호화 및 사용자 인증 정보 →
인증서 설치 → CA 인증서 → (경고 화면에서) 무시하고 설치 → 다운로드 폴더의 파일 선택**

- 경로는 제조사/안드로이드 버전마다 다르다. 못 찾겠으면 설정 검색창에 **"인증서 설치"** 또는
  **"CA 인증서"** 를 입력한다.
- 삼성 One UI: 설정 → 보안 및 개인정보 보호 → 기타 보안 설정 → 인증서 설치
