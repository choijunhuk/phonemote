...
> **실기기 이슈 — 화면 회전 잠금**: 회전 잠금이 켜져 있으면 폰을 물리적으로 가로로 들어도
> Chrome은 계속 `portrait-primary` 를 보고한다. 그러면 Normalizer가 90°를 덜 돌려
> **pitch와 roll이 뒤바뀐다.** 그래서 컨트롤러에 "잡는 방향"(가로 고정 / 세로 고정 / 자동)
> 선택을 두고 **기본값을 가로 고정**으로 한다. canonical 자세 자체가 가로이므로(5.1),
...
- `SwingDetector` 상수 (**실기기 측정으로 재조정**): `SWING_THRESHOLD 25 m/s²`,
  `SWING_MAX 90 m/s²`, `SWING_MIN_WINDOW 25 ms`, `SWING_CAPTURE_WINDOW 80 ms`,
  `SWING_COOLDOWN 300 ms`, `SWING_DECAY_RATIO 0.5`.
  `strength = clamp((peak − THRESHOLD) / (MAX − THRESHOLD), 0, 1)`.
  - 스펙 초안의 15/40은 실기기와 맞지 않았다. 실제 스윙 피크는 **100 m/s² 부근**이라
    상한 40에서는 모든 스윙이 strength 1.0이 되어 공이 항상 최고 속도로 날아갔다.
  - 고정 윈도우를 끝까지 기다리면 그 길이만큼 **모든 타격이 늦어진다.** 피크가 지나
    `peak × 0.5` 아래로 떨어지면 즉시 발화하고, 윈도우는 가라앉지 않는 스윙의 보호장치로만 쓴다.
