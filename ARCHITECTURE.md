...
- `DeviceMotionEvent.rotationRate`: **실기기에서는 `alpha`, `beta`, `gamma` 가 각각
  `x`, `y`, `z` 축 각속도다.** W3C 스펙은 z, x, y 라고 적고 있지만 이 기기는 그렇지 않다.
  자세각(alpha/beta/gamma)은 스펙대로이고 **각속도만** 다르다.

  > **측정 근거.** 연속된 자세각으로 회전행렬을 만들어 `R1ᵀR2` 에서 진짜 body 각속도를 뽑고
  > 보고값과 대조했다. 5개 세션, 움직임이 충분한 샘플 수백 개의 상관계수:
  > `true x ↔ alpha +0.92`, `true y ↔ beta +0.78`, `true z ↔ gamma +0.90`.
  > 개별 동작으로도 같은 결론이다 — 좌우로 눕히기(z축 회전)는 `gamma −228`,
  > 앞뒤로 끄덕이기(x축 회전)는 `alpha +47/−83`, 뒤집기(y축 회전)는 `beta −81` 로 나왔다.
  >
  > **증상.** 스펙대로 믿었을 때 끄덕임(x축)이 `alpha` 로 들어와 **roll 로 새어나갔고**,
  > canonical `pitch` 는 거의 반응하지 않았다. 포인터의 상하가 동작하지 않은 원인이 이것이다.
  > 좌우는 z축 회전이 `gamma` → `yaw` 로 흘러 우연히 그럴듯하게 동작했다.
- `DeviceMotionEvent.acceleration`: 기기 축 `x, y, z` 성분, 중력 제외, m/s².

벡터로 다룰 때:
`a_d = (ax, ay, az)`, `ω_d = (rotationRate.alpha, rotationRate.beta, rotationRate.gamma)`.

`ω_d` 의 순서가 스펙과 다른 이유는 바로 위 측정 근거를 볼 것. 다른 기기에서 어긋나면
이 줄과 5.6 표를 먼저 고치고 코드를 따라오게 한다.
...
|---|---|---|---|
| 0 portrait-primary | `+α` | `−β` | `−γ` |
| 1 landscape-primary | `−β` | `−α` | `−γ` |
| 2 portrait-secondary | `−α` | `+β` | `−γ` |
| 3 landscape-secondary | `+β` | `+α` | `−γ` |

`roll` 축은 화면 회전축과 같으므로 항상 `−γ` 다 (좋은 자체 검산 포인트).
초안에는 `−α` 라고 적혀 있었는데, 그건 스펙이 말하는 채널 순서를 믿었기 때문이다.
...
→ `β < 0` → `yaw = −β > 0`. 규약(오른쪽 겨눔 = `yaw +`)과 일치. ✔
그리고 앞뒤로 끄덕이면 x축 회전이므로 `α` 가 움직이고 `pitch = +α` 로 상하가 반응한다. ✔
...
- `SwingDetector` — **각속도(|ω|)로 구간을 나눈다.** 가속도가 아니다.
  ...
  - **방향은 회전축이 아니라 폰의 먼 끝이 간 곳**이고, 그 수평 성분은 **yaw + roll** 이다.
    ...
    수평 스윕을 만든다. 둘을 더하면 두 그립을 모두 포용하고, 중간으로 쥔 손은 양쪽에서 몫을
    받는다. pitch는 어느 그립이든 상하 성분이다.

    > 이 항목의 근거로 한때 "실측 스윙이 roll 우세"라고 적혀 있었으나 **그건 5.4의 각속도 축
    > 오해에서 나온 값이었다.** 같은 기록을 바로잡고 다시 읽으면 순 이동 `x 2, y −36` 의
    > 내리치는 스윙이다. roll을 포함하는 이유는 위의 기하이지 그 측정이 아니다. 겨냥 축(`−Z`) 위의 점은 `ω × r` 로 움직이고,
...
| Freeze Frame 상태 머신 | Vitest (그립 보정·유지 판정·목숨·순서 점수·방 변경) | 확장 5단계 |
| **브라우저 종주** | 개발 전용 핸들로 게임 루프와 가짜 폰을 합성 시계로 돌려 한 판 완주 | 확장 5단계 |
...
**브라우저 종주**는 `import.meta.env.DEV` 에서만 붙는 두 개의 핸들(`window.phonemote`,
`window.phonemoteFake`)로 한다. 자동화 브라우저에는 `requestAnimationFrame` 이 아예 없고
타이머도 억제되어 Phaser 루프와 60Hz 가짜 폰이 함께 멎는다. 프레임 주입과 `game.step` 을
호출자가 정한 시계로 직접 돌리면 한 판이 수 ms 만에 끝나고 결과가 재현 가능하다.
이것으로 확인되는 것은 **파이프라인 전체**(가짜 센서 프레임 → 정규화 → 검출 → GameAction →
규칙 → 렌더)이지, 실제 센서가 아니다.
...
| D27 | `rotationRate` 채널을 스펙(z,x,y)이 아니라 **실측(x,y,z)** 으로 해석 | 회전행렬로 뽑은 진짜 body 각속도와의 상관계수가 0.92/0.78/0.90 으로 대각선에 몰렸다. 스펙을 믿는 동안 끄덕임이 roll로 새어 포인터 상하가 죽어 있었다 |
| D26 | 측정 도구는 시간이 아니라 **조건**으로 진행 | 첫 실측에서 스윙과 뒤집기 두 단계가 창을 벗어났다. 이제 정지하면 자세를 찍고, 움직이면 동작을 찍는다 |
| D28 | 테니스 공 속도를 `strength` 가 아니라 `peakRate` 에서 산출 | `strength` 는 900°/s 에서 포화하는데 실제 스윙 피크가 900~1250 이다. 모든 타구가 최고속으로 나가던 원인이고, "공이 너무 빨라서 못 친다"의 실체다 |
| D29 | 타격 후 120ms 안에 더 강한 버스트가 오면 **그 타격을 대체** | 스트로크는 백스윙 다음에 본 스윙이고, 검출기는 둘 다 쏜다. 먼저 온 백스윙이 공을 치고 본 스윙은 존을 벗어나 "너무 늦었음"이 되던 것을 없앤다 |
| D30 | 히트존 자체를 라켓으로 그림 | 코트의 1/3 폭 존 한가운데에 14px 라켓을 그려두면, 창이 열려 있는데 닫힌 것처럼 보인다. 밝아지는 밴드가 "지금"을 말하는 유일한 신호다 |
| D31 | 아웃은 베이스라인이 아니라 그 너머 스트립 통과 | 공이 선을 넘자마자 멈췄다가 순간이동하던 것을 없애고, 눈에 보이게 나가는 동안은 아직 칠 수 있게 남긴다 |
| D32 | Freeze Frame 판정을 순간 각도 → **연속 유지(dwell)** 로 | 한 순간의 각도는 지나가던 손을 맞다고 하고, 먼저 도착해 숨 쉰 손을 틀렸다고 한다. 자세 게임이면 정지를 재야 한다 |
...
| D51 | 그립 캡처 창을 **샘플 개수 → 시간(500ms)** 으로 | 네 게임이 전부 '30 샘플'로 창을 잡았는데 그건 60Hz에서만 0.5초다. 15Hz면 2초, 100Hz면 0.3초 — 손을 고쳐 잡고 A를 누른 사람이 얻는 영점이 자기 손이 아니라 자기 폰에 따라 달라졌다 (11.2) |
| D52 | 볼링 공 속도를 릴리스 **순간 속도 → 트리거를 쥔 동안의 스윙 최고 속도**로, 스윙 중 **실시간 파워 게이지** (D49 개정) | 실기기 테스트에서 "스윙하는 느낌이 안 난다". 원인은 속도가 트리거를 떼는 한 프레임의 |ω|로만 정해진 것이었다 — 앞스윙 최고점을 조금만 지나 떼도 세게 휘두른 공이 최저 속도로 굴렀고, 휘두르는 동안 화면은 아무것도 반응하지 않았다. 트리거를 쥔 동안의 최고 |ω|를 기록해, 그 최고점이 릴리스 450ms 이내면 그 값을 쓴다(앞스윙 피크→릴리스는 수백 ms, 20Hz 검출 지연 50~102ms 포함). 그보다 오래됐으면 망설이는 동안의 백스윙이므로 순간값을 쓴다. 릴리스 시점은 여전히 버튼 엣지다 — D49가 풀려던 문제(약한 딜리버리가 아무 일도 안 일으킴)는 그대로 막힌다 |
| D53 | 트리거 없이 던질 때 **첫 스윙에서 바로 굴린다** — 백스윙 짝짓기 폐기 (D49 폴백 개정) | 실기기 테스트: "슥 올렸을 때 공이 안 놓아지고, 내려갈 때 놓아져서 이상하다". 폴백은 첫 스윙을 백스윙으로 붙잡고 1.2초 안의 다음 스윙을 투구로 굴렸다 — 위로 퍼 올리는 투구는 붙잡히기만 하고, 팔을 내리는 동작이 투구로 굴렀다. 폰으로 볼링 흉내를 내는 자연스러운 동작은 퍼 올리기 하나이므로 첫 스윙이 곧 투구다. 검출기는 피크 감쇠에서 발화하므로 공은 올리는 스윙의 꼭대기 근처에서 나간다. 뒤로 뺐다가 앞으로 던지는 Wii식 동작은 트리거 경로(D49, D52)가 그대로 맡는다 |
